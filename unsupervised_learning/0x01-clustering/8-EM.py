...
def expectation_maximization(X, k, iterations=1000, tol=1e-5, verbose=False):
    ...
    - pi is numpy.ndarray of shape (k,) containing the priors for each cluster
    ...
    - log is the log likelihood of the model
    """
    ...
    for i in range(iterations):
        if verbose and (i % 10 == 0):
            print('Log Likelihood after {} iterations: {}'.
                  format(i, log.round(5)))

        pi, m, S = maximization(X, g)
        g, log = expectation(X, pi, m, S)

        if abs(l_prev - log) <= tol:
            break
        l_prev = log

    if verbose:
        print('Log Likelihood after {} iterations: {}'
              .format(i + 1, log.round(5)))
    return pi, m, S, g, log
